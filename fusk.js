...
(function () {
  ...
  // ─── Välj bästa svar ─────────────────────────────────────────────────────
  function autoAnswer() {
    // Hitta alla synliga svarsknappar
    ...
    // Välj det längsta svaret (mest aggressivt i Bad News)
  ...
  function init() {
    ...
    var iv = setInterval(function () {
      attempts++;
      if (attempts > 40) { clearInterval(iv); return; } // 20 sekunder max

      var store = getStore();
      if (!store || store.state.loading) return; // Vuex finns men spelet laddar
